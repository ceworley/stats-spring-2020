---
title: "Mat-181 Exam 1 Spring 2020 Retake"
output:
  pdf_document:
    includes:
...
On each problem you missed points on the original test, you can get back half the points by doing the corresponding problem here perfectly.
...
question & available points & points original test & additional points earned \\ \hline \hline
1 & 5 & & \\ \hline
2 & 5 &  & \\ \hline
3 & 10 &  & \\ \hline
4 & 10 &  & \\ \hline
...
a. The age of a caterpillar  \hspace{10pt} (quantitative/categorical) \vspace{20pt}
b. Whether a caterpillar is fuzzy \hspace{10pt} (quantitative/categorical) \vspace{20pt}
c. The weight of a caterpillar \hspace{10pt} (quantitative/categorical) \vspace{20pt}
d. The color of a caterpillar  \hspace{10pt} (quantitative/categorical) \vspace{20pt}
e. The length of a caterpillar \hspace{10pt} (quantitative/categorical) 
...
while(T){
  ...
  xbar = mean(x)
  s = sd(x)
  if(as.integer(mu)==mu && as.integer(xbar)==xbar && sig!=0 && s!=0 && mu != xbar){break}
}
```
...
n = 500 #sample(c(20,40,50,60,80,100),1)*5
offset = 110 #runif(1,0,60)
rng = 60 #sample(c(3,5,10,20,40,100),1)
...
library(latex2exp)
set.seed(123)
mu = 333  #signif(10**runif(1,-1,3),2)
sd = 111 #signif(mu/3*runif(1,0.1,1),1)
