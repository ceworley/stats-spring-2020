...
* This is a practice exam. It is due **March 10**. 
* The actual exam will be on **Thursday March 12**.
* The formula sheet will be provided on the actual exam.
* Enjoy!
...
a. What is probability a randomly selected marble is checkered? 
$$\text{Pr(checkered)} =  \phantom{\text{Pr(checkered)}}$$
\vfill
b. What is the probability a randomly selected marble is blue?
$$\text{Pr(blue)} = \phantom{\text{Pr(blue)}} $$
\vfill
c. If you know a randomly selected marble is checkered, then what is the probability it is blue?
$$\text{Pr(blue GIVEN checkered)} = \phantom{\text{Pr(blue GIVEN checkered)}} $$
\vfill
...
Jan will paint 4 different rooms different colors. There are 9 colors to choose from. How many options does Jan have for how the apartment will be painted?

\vfill

## Question 6 (10 pts)

Karl will select 4 toppings for his pizza. There are 12 different toppings available. How many different pizzas are possible?
...
When Emmaline is shooting archery, each arrow has an 70% chance of hitting the bullseye. Emmaline will shoot 12 arrows. We are interested in how many bullseyes Emmaline will hit.
...
\vfill
c. What is the probability that Emmaline hits the bullseye at least 10 times?
$$\text{Pr}(X\ge 10) = \phantom{\text{Pr}(X\ge 10)} $$
\vfill
...
In a game, there is a 30% chance Bob will win. If Bob wins, there is an 60% chance he is happy. If Bob does **not** win, there is a 20% chance he is happy. After playing the game, Bob is **not** happy; what is the probability that Bob won the game?
...
Candice has two upcoming tennis matches. She has a 75% chance of winning the first match and a 20% chance of winning the second match.
