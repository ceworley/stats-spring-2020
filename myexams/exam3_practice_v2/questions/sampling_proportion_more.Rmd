...
A very large population has a population proportion $p=`r p`$. When a random sample of size `r n` is taken, what is the probability that the **sample proportion** ($\hat{p}$) is more than `r phat`? 
...
polygon(c(Z[Z>=z],3.5,z),c(dnorm(Z[Z>=z]),0,0),col=rgb(0.4,0.4,0.4,0.4))
```
