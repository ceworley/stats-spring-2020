...
Let each trial have a chance of success $p=`r p`$. If `r n` trials occur, what is the probability of getting at least `r ks[1]` but less than `r ks[2]` successes? 

In other words, let $X\sim \text{Bin}(n=`r n`,\,p=`r p`)$ and find $P(`r ks[1]` \le X < `r ks[2]`)$.
...
$$P(`r ks[1]`\le X < `r ks[2]`) = `r round(pnorm(z2),4)` - `r round(pnorm(z1),4)` = `r Pr` $$


Meta-information
============
extype: string
exsolution: $P(`r ks[1]`\le X < `r ks[2]`) = `r Pr` $
exname: binomial normal approximation al lt
extol: 0.01
