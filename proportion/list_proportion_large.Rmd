...
names = read.csv("yob2018.txt",header=FALSE)
numnames = length(names[,1])
while(TRUE){
  who = sample(1:numnames,1)
  name = names[who,1]
  gender = names[who,2]
  freq = names[who,3]
  if(freq>1000){
...
`r name` is practicing [free throws](https://en.wikipedia.org/wiki/Free_throw). `r HeShe` has recorded the results of many free throws.
...
Answerlist
--------
* How many shots did `r name` attempt? (Evaluate $n$.)
* How many shots were successful? (Evaluate $n_\text{s}$.)
* How many shots were failures? (Evaluate $n_\text{f}$.)
* What proportion of `r name`'s shots successful? (Evaluate $\hat{p}$.)
* What proportion of `r name`'s shots failures? (Evaluate $\hat{q}$.)
...
names(mydf) = c("i","hit_or_miss","hit_or_miss=Hit","hit_or_miss=Miss")
