...
while(T){
  ...
  lets = strsplit("BCDFGHJKMQRSVWXY",split=NULL)
  ...
  wrd = sample(c("lands over","lands above","lands on a value more than"),1)
...
Evaluate $`r xl`$ such that $P(`r Xl`>`r xl`) = `r ra`$. In other words, determine an lower boundary such that a normal spinner with mean `r mu` and standard deviation `r sig` `r wrd` that boundary `r ra*100`% of the time. 


Solution
========
First, draw a sketch. We can label the $`r Xl`$ axis by adding integer multiples of `r sig` to `r mu`. We know to shade the right because high values of $`r Xl`$ satisfy the condition $`r Xl`>`r xl`$ (regardless of the exact value of $`r xl`$). We don't know exactly where to place the boundary, but we know the right area is `r ra`.
...
text(2,0.3,TeX(sprintf("$P(%s>%s) = %.2f$",Xl,xl,ra)))
...
As an intermediate step, we find $z$ such that $P(Z<z)=`r la`$. 
