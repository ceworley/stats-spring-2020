...
while(T){
  ...
  lets = strsplit("BCFGHJKMQRSVWXY",split=NULL)
  ...
  wrd = sample(c("lands outside","lands beyond"),1)
...
Answerlist
--------
* Evaluate $d$ such that $P\left(\big|`r Xl`-`r mu`\big|>d\right) = `r ta`$. In other words, determine the distance from mean of two boundaries such that this normal spinner `r wrd` those boundaries `r ta*100`% of the time. 
* Let $`r xl`_1 = `r mu`-d$, such that $P\left(`r Xl` < `r xl`_1\right) = \frac{`r ta`}{2}$. Evaluate $`r xl`_1$.
* Let $`r xl`_2 = `r mu`+d$, such that $P\left(`r Xl` > `r xl`_2\right) = \frac{`r ta`}{2}$. Evaluate $`r xl`_2$.


Solution
========
First, draw a sketch. We can label the $`r Xl`$ axis by adding integer multiples of `r sig` to `r mu`. We know to shade the the outsides because values far from `r mu`  satisfy the condition $\big|`r Xl`-`r mu`\big|>d$ (regardless of the exact value of $d$). We don't know exactly where to place the boundaries, but we know the two-tail area is `r ta`.
...
text(0,0.45,TeX(sprintf("$P(|%s-%.3g|>d) = %.2f$",Xl,mu,ta)))
```
\


As an intermediate step, let's find $z$ such that $P(|Z|>z)=`r ta`$. First, we need to evaluate $P(Z<z)$.
...
exname: Get bounds from twotailarea
