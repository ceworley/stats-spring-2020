...
That spinner will be spun `r n` times, and the **sum** of those rolls will be recorded. What is the probability the sum will be **outside** `r d` units from the expected value?
...
$$P\left(\left|\sum X - `r mut` \right|> `r d` \right) ~=~ P\left(\big|Z\big|>`r z`\right) ~=~ `r ta` $$
