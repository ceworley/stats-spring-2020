...
To help you along, I will calculate the mean and standard deviation of **single** rolls. The formulas for an $N$-sided die are can be derived from the formulas for [discrete uniform distribution](https://en.wikipedia.org/wiki/Discrete_uniform_distribution).
$$\mu_{\tiny X} ~=~ \frac{N+1}{2} ~=~ \frac{`r N`+1}{2} ~=~ `r mu` $$
$$\sigma_{\tiny X} ~=~ \sqrt{\frac{N^2-1}{12}} ~=~ \sqrt{\frac{`r N`^2-1}{12}} ~=~ `r sig`$$
...
$$\sum X \sim \mathcal{N}(`r mut`,\,`r sigt`) $$
...
$$z ~=~ \frac{(\sum x)-\mu_{\tiny \sum X}}{\sigma_{\tiny \sum X}} ~=~ \frac{`r xt`-`r mut`}{`r sigt`} ~=~ `r z`$$
